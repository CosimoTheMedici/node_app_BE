var dbConn = require("../configs/db.config.js").default;
...
Property.getAllProperties = (callBack) => {
  dbConn.query("SELECT * FROM properties", (error, results, fields) => {
    if (error) {
      callBack(error);
    }
...
Property.getAgentPropertiesByEmail = (id, callBack) => {
  try {
    ...
    const conditionList = [id];
    const result = dbConn.query(
      `SELECT * FROM properties WHERE FIND_IN_SET((SELECT agent_id FROM agents WHERE agent_email= ?), property_agent)`,
...
Property.updatePropertyAgentsByID = (data, id, callBack) => {
  try {
    ...
    const result = dbConn.query(
      `UPDATE properties SET property_agent = ? WHERE properties.property_id = ?;`,
      conditionList,
      (error, results, field) => {
        if (error) {
          return callBack(error);
