...
class Payments {
  ...
  static async getPaymentByCreatedID(id, callBack) {
    try {
      ...
      const result = db.query(
        `SELECT * FROM payments LEFT JOIN properties ON properties.property_id = payment_propertyName LEFT JOIN units ON payment_unitName= units.unit_id WHERE createdBy  = ?`,
        conditionList,
        (error, results, field) => {
          if (error) {
  ...
  static async getPaymentLastRecord(data, callBack) {
    const conditionList = data;
    db.query(
      `SELECT * FROM transactions WHERE transaction_propertyID = ? AND transaction_unitID = ? ORDER BY transaction_date DESC LIMIT 1;`,
      conditionList,
      (error, results, field) => {
        if (error) {
          return callBack(error);
