...
class Transaction {
  ...
  static async getLastTranDataBypropIDunitID(conditionList) {
    return new Promise((resolve, reject) => {
      ...
      db.query(
        "SELECT * FROM transactions WHERE transaction_date = (SELECT MAX(transaction_date) FROM transactions WHERE transaction_propertyID = ? AND transaction_unitID = ?) AND transaction_propertyID = ? AND transaction_unitID = ?",
        conditionList,
        (error, results) => {
          if (error) {
            reject(error);
