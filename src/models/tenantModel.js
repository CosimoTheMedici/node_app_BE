...
class Tenant {
  ...
  static async getAllTenantByPropertyID(email, callBack) {
    ...
    db.query(
      `SELECT * FROM tenants WHERE FIND_IN_SET(propertyID, (SELECT agent_properties FROM agents WHERE agent_email= ? ))`,
  ...
  static async getAllTenantByAgentOwner(callBack) {
    db.query(
      `SELECT * FROM tenants LEFT JOIN units ON tenants.unitName = units.unit_id WHERE propertyID IN(SELECT property_id FROM properties)`,
      //db.query(`SELECT * FROM utility_Charges WHERE charge_name = ? and charge_property_id IN (?)`,
