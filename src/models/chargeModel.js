...
class Charge {
  ...
  static async getConsumptionLastRecordo(data, callBack) {
    ...
    db.query(
      `SELECT MAX(reading_id) AS latest_id FROM reading WHERE reading_propertyID = ? and reading_unitID =?   and reading_utilityTypeID = ?`,
      conditionList,
      (error, results, field) => {
        if (error) {
          return callBack(error);
  ...
  static async createUtilityCharge(data, callBack) {
    const conditionList = [data];
    db.query(
      `INSERT INTO utility_Charges SET ?`,
      conditionList,
      (error, results, field) => {
        if (error) {
          return callBack(error);
  ...
  static async getChargesByType(chargeType) {
    try {
      const result = await db.query(
        `SELECT * FROM utility_Charges WHERE charge_type = ?`,
        [chargeType]
      );
      return result;
    } catch (error) {
  ...
  static async getchargesByAgentPropertiesByEmail(data, callBack) {
    try {
      ...
      //const result = db.query(`SELECT * FROM utility_Charges WHERE charge_property_id IN (SELECT agent_properties FROM agents WHERE agent_email=?)`,
      const result = db.query(
        `SELECT * FROM utility_Charges WHERE FIND_IN_SET(charge_property_id,(SELECT agent_properties FROM agents WHERE agent_email= ? ))`,
  ...
  static async getutilityChargesData1(data, callBack) {
    try {
      ...
      //const result = db.query(`SELECT * FROM utility_Charges WHERE charge_name = ? and charge_property_id IN (?)`,
      const result = db.query(
        `SELECT * FROM utility_Charges WHERE charge_property_id= ? AND charge_name=? AND charge_type=?`,
        conditionList,
        (error, results, field) => {
          if (error) {
            return error;
  ...
  static getutilityChargesData1(conditionList) {
    try {
      ...
      const result = db.query(
        `
        SELECT ${columnList} FROM utility_Charges WHERE charge_property_id= ? AND charge_name =? AND charge_type = ?`,
        conditionList
      );
  ...
  static async getutilityChargesData(conditionList) {
    return new Promise((resolve, reject) => {
      db.query(
        "SELECT * FROM utility_Charges WHERE charge_property_id = ?  AND charge_type = ?",
        conditionList,
        (error, results) => {
          console.log("query");
          if (error) {
